...
router.get('/empresas', async (req, res) => {
  try {
    const [empresas] = await db.query(`
      SELECT de.id, de.nombre_empresa AS nombre, de.nit, de.direccion, 
...
// ✅ Asociar usuario con empresa (solicitud pendiente)
router.post('/asociar', async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'Error al asociar' });
  }
});
...
router.get('/usuarios/:usuarioId/empresas', async (req, res) => {
  ...
  try {
    const [rows] = await db.query(`
      SELECT de.nombre_empresa AS nombre, de.nit, u.correo AS email,
             ec.estado
